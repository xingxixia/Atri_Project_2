...
def main():
    ...
    while True:
        ...
        prefix_embed = torch.randn(
            (input_embeds.size(0), prefix_length, hidden_size),
            device=input_embeds.device,
            dtype=input_embeds.dtype
        )
        inputs_embeds = torch.cat([prefix_embed, input_embeds], dim=1)# 在这里呢，imputs可以是当前usr输入，prefix_embed可以是压缩过后的历史文本，或许这样可以增大模型感受野吧


        streamer = TextIteratorStreamer(tokenizer, skip_prompt=True, skip_special_tokens=True)
        generate_kwargs = dict(
            inputs_embeds=inputs_embeds,   # 插入embedding的接口
            streamer=streamer,
            max_new_tokens=512,
            do_sample=True,
            temperature=0.7,
